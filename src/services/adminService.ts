...
const mapRequest = (row: Record<string, unknown>): AdminRequest => ({
  ...
  requestedRole:
    row.requested_role as AdminRequest['requestedRole'],
...
const merchantCategory = (merchantLocalId?: string) =>
  merchants.find((item) => item.id === merchantLocalId)?.categoryId ??
  'other'

export const adminService = {
  async fetchOverview(): Promise<AdminOverview> {
    const [profilesResult, ordersResult, requestsResult] =
      await Promise.all([
  ...
  async assignMerchant(
    ...
    const { data, error } = await supabase.rpc(
      'admin_assign_merchant',
      {
        p_profile_id: profileId,
        p_merchant_local_id: merchantLocalId,
      },
    )
